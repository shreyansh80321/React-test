    ...
    <div className="max-w-6xl mx-auto p-4 lg:p-10 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-1 bg-white rounded-2xl shadow-lg p-6">
          {cartItem?.length ? (
            cartItem.map((item) => <CartTilePage />)
          ) : (
            <h1 className="text-gray-500 text-lg font-medium">No item found</h1>
          )}
        </div>
        <div className="w-full lg:w-1/3 bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-6">
          ...
          <div className="flex gap-2 mt-4">
            <button className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-800 transition-colors duration-300 cursor-pointer">
              Checkout
            </button>
            <button
